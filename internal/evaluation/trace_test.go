...
// and the bytes are what a recorded payload carries into an audit record.
...
// the admitted documents, tri-state evidence, and this evaluator release.
func TestTraceIsADeterministicOrderedRecord(t *testing.T) {
	pack := map[string]any{
		...
		"exceptions": []any{
			map[string]any{"id": "x1", "when": unknownCondition(), "effect": "suppress-rule", "targetRule": "r1", "onUnknown": "ignore"},
			map[string]any{"id": "x2", "when": literalCondition(false), "effect": "escalate", "onUnknown": "ignore"},
		},
		"rules": []any{
			...
			map[string]any{"id": "r3", "when": unknownCondition(), "outcome": "a", "onUnknown": "ignore"},
		},
	}
	want := `[{"stage":"applicability","condition":"true"},` +
		`{"stage":"exception","id":"x1","condition":"unknown","onUnknown":"ignore"},` +
		`{"stage":"exception","id":"x2","condition":"false"},` +
		`{"stage":"rule","id":"r1","condition":"false"},` +
		`{"stage":"rule","id":"r2","condition":"true","outcome":"a"},` +
		`{"stage":"rule","id":"r3","condition":"unknown","onUnknown":"ignore"}]`
...
func TestSuppressedAndEvaluatedRulesShareOneRecord(t *testing.T) {
	pack := map[string]any{
		"outcomes": []any{map[string]any{"id": "a"}, map[string]any{"id": "b"}},
		"exceptions": []any{
			map[string]any{"id": "x", "when": literalCondition(true), "effect": "suppress-rule", "targetRule": "r1", "onUnknown": "ignore"},
		},
		"rules": []any{
			map[string]any{"id": "r1", "when": literalCondition(true), "outcome": "a", "onUnknown": "ignore"},
	...
	want := `[{"stage":"exception","id":"x","condition":"true","effect":"suppress-rule"},` +
		`{"stage":"rule","id":"r1","condition":"not-evaluated","suppressed":true},` +
...
// Evidence inspection leaves no trace entry. §8 step 2 inspects tri-state
// presence without evaluating any condition, so there is no evaluation to
// record, and inventing an entry would trace something that never ran; its
// findings surface as disposition reasons instead. The one authored stage
// before the walk stopped is the whole record, and the unknown reason beside it
...
func TestEvidenceInspectionIsNeverTraced(t *testing.T) {
	pack := map[string]any{
		...
		"evidenceRequirements": []any{
			map[string]any{"id": "e1", "required": true},
		},
		"rules": []any{
			map[string]any{"id": "r1", "when": literalCondition(true), "outcome": "a", "onUnknown": "ignore"},
		},
	}
	evaluator := &evaluator{evidence: map[string]tri{"e1": triUnknown}, budget: DefaultCoreWorkLimit}
	...
	if disposition.Kind != "unresolved" || !reflect.DeepEqual(disposition.Reasons, []string{"unknown"}) {
		t.Fatalf("unknown required evidence blocks resolution: %+v", disposition)
	}
...
// before the expensive rule interrupts the walk, so a payload that leaked the
// partial record would carry a visible entry rather than an empty slice this
// assertion cannot tell from absence.
func TestRefusedEvaluationReportsNoTrace(t *testing.T) {
  ...
  "decision": {
    "intent": "Exercise the no-partial-trace rule of a refused evaluation.",
