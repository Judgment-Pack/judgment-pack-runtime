...
// packs diagram: selection and refusal behavior, one exit code per failure
// class. The mapping itself is specified by the goldens in internal/diagram;
// these tests pin the command around it.
func TestPacksDiagramSelectsRendersAndRefuses(t *testing.T) {
	minimalPack := `{"specVersion":"0.2.0-draft","id":"https://example.invalid/p","version":"0.0.1",` +
		`"outcomes":[{"id":"allow","label":"Allow"}],` +
		`"rules":[{"id":"r","when":{"op":"fact","path":"/x","operator":"equals","value":true},"outcome":"allow"}]}`

	t.Run("one pack renders without --id", func(t *testing.T) {
		config := writeProjectFixture(t, `{"configVersion":"1","packs":{"only":{"path":"packs/p.pack.json"}}}`,
			map[string]string{"packs/p.pack.json": minimalPack})
		code, stdout, stderr := runTest(t, []string{"packs", "diagram", "--config", config}, "")
		if code != 0 || stderr != "" {
			t.Fatalf("exit=%d stderr=%q", code, stderr)
		}
		for _, wanted := range []string{"flowchart TD", `out_allow(["Allow"])`, "rule_r --> out_allow"} {
	...
	t.Run("several packs demand --id", func(t *testing.T) {
		config := writeProjectFixture(t, `{"configVersion":"1","packs":{`+
			`"a":{"path":"packs/p.pack.json"},"b":{"path":"packs/p.pack.json"}}}`,
			map[string]string{"packs/p.pack.json": minimalPack})
		code, _, stderr := runTest(t, []string{"packs", "diagram", "--config", config}, "")
		if code != result.ExitInvocation || !strings.Contains(stderr, "--id") {
			t.Fatalf("exit=%d stderr=%q", code, stderr)
		}
	})

	t.Run("unknown id is refused with the configured ids", func(t *testing.T) {
		config := writeProjectFixture(t, `{"configVersion":"1","packs":{"only":{"path":"packs/p.pack.json"}}}`,
			map[string]string{"packs/p.pack.json": minimalPack})
		code, stdout, stderr := runTest(t, []string{"packs", "diagram", "--config", config, "--id", "nope"}, "")
		if code != result.ExitUnsupported || !strings.Contains(stdout+stderr, "only") {
			t.Fatalf("exit=%d stdout=%q stderr=%q", code, stdout, stderr)
		}
	})

	t.Run("duplicate members are refused as invalid", func(t *testing.T) {
		config := writeProjectFixture(t, `{"configVersion":"1","packs":{"dup":{"path":"packs/dup.pack.json"}}}`,
			map[string]string{"packs/dup.pack.json": `{"id":"x","id":"y","version":"1.0.0"}`})
		code, _, stderr := runTest(t, []string{"packs", "diagram", "--config", config}, "")
		if code != result.ExitInvalid || !strings.Contains(stderr, "duplicated") {
			t.Fatalf("exit=%d stderr=%q", code, stderr)
		}
	})

	t.Run("a carrier resource limit is operational, not invalid", func(t *testing.T) {
		deep := strings.Repeat(`{"a":`, 200) + "1" + strings.Repeat("}", 200)
		config := writeProjectFixture(t, `{"configVersion":"1","packs":{"deep":{"path":"packs/deep.pack.json"}}}`,
			map[string]string{"packs/deep.pack.json": deep})
		code, _, stderr := runTest(t, []string{"packs", "diagram", "--config", config}, "")
		if code != result.ExitIO {
			t.Fatalf("exit=%d stderr=%q", code, stderr)
		}
	...
	t.Run("a huge number renders like every other surface reads it", func(t *testing.T) {
		big := `{"id":"https://example.invalid/big","version":"1.0.0",` +
			`"outcomes":[{"id":"o","label":"O"}],` +
			`"rules":[{"id":"r","when":{"op":"fact","path":"/x","operator":"equals","value":1e400},"outcome":"o"}]}`
		config := writeProjectFixture(t, `{"configVersion":"1","packs":{"big":{"path":"packs/big.pack.json"}}}`,
			map[string]string{"packs/big.pack.json": big})
		code, stdout, stderr := runTest(t, []string{"packs", "diagram", "--config", config}, "")
		if code != 0 || stderr != "" {
			t.Fatalf("exit=%d stderr=%q", code, stderr)
		}
		if !strings.Contains(stdout, "equals 1e400") {
			t.Fatalf("the document's own numeric literal must render:\n%s", stdout)
		}
	})
}
