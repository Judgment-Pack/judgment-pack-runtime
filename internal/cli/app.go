...
func (a *App) rootCommand() *cobra.Command {
	root := &cobra.Command{
		...
		Long:          "Judgment Pack Specification tooling. The spec commands validate JPS documents; they do not evaluate decisions or authorize actions. The experimental namespace is the one exception: it evaluates, under one exact contract this runtime claims evaluator conformance to (CONFORMANCE.md, JPS §3.4.1), on a surface that may change without notice. No command authorizes anything.",
		SilenceErrors: true,
		SilenceUsage:  true,
...
func (a *App) experimentalCommand() *cobra.Command {
	experimental := &cobra.Command{
		...
		Long:  "Experimental operations (ADR-0007, ADR-0010, ADR-0011). \"Experimental\" here is a stability statement and not a conformance one: this surface may change or be removed without compatibility promise. The evaluator implements the evaluator conformance class of JPS Core 0.2.0-draft -- the §8.2 input preflight, the §8.3 portable disposition, the §8.4 error classes, and the §10 limits -- and this runtime claims evaluator conformance against that exact specVersion and the evaluation corpus published for it, in the one form §3.4.1 permits and only there: see CONFORMANCE.md for the claim and its exact scope. The contract is applied to a pack of either bundled version, whatever the pack itself declares, so every payload names the contract's own version as evaluatorSpecVersion beside the pack's specVersion; nothing is claimed under 0.1.0-draft, which defines no evaluator class. The claim asserts nothing about any pack, any facts, any evidence, or the consequences of acting on a disposition (§3.5).",
		Args:  cobra.NoArgs,
		RunE: func(command *cobra.Command, _ []string) error {
			return command.Help()
		},
...
// runtime's claim cites: the run produces results, which §3.4.1 makes the
// required and non-exhaustive evidence for that claim rather than the claim
// itself, and every payload points at CONFORMANCE.md in band.
...
func (a *App) mcpCommand() *cobra.Command {
	return &cobra.Command{
		...
		Long:  "Run a Model Context Protocol server on standard input and output. It exposes the offline validation, conformance, and description operations as MCP tools, plus one evaluation tool on this runtime's experimental surface (experimental_evaluate; ADR-0007, ADR-0011), whose evaluator carries the §3.4.1 claim of CONFORMANCE.md and whose surface may still change without notice. It holds no credential, opens no network connection, and authorizes nothing.",
		Args:  cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
