...
func (a *App) mcpCommand() *cobra.Command {
	return &cobra.Command{
		Use:   "mcp",
		Short: "Serve the offline validator and the experimental tools to an MCP client over stdio",
		Long:  "Run a Model Context Protocol server on standard input and output. It exposes the offline validation, conformance, and description operations as MCP tools, plus six tools on this runtime's experimental surface (experimental_evaluate, experimental_test_packs and experimental_test_graphs, which reach the evaluator, experimental_list_graphs and experimental_get_graph, which serve the graph convention's inventory and documents read-only, and experimental_validate_expectations, which checks proposed exact dispositions against §8.3 and reaches nothing at all; ADR-0007, ADR-0011, ADR-0021, ADR-0026, ADR-0029, ADR-0035), whose surfaces may still change without notice. That evaluator's conformance claim is stated, in full and only, in CONFORMANCE.md; this text states no claim. It holds no credential, opens no network connection, and authorizes nothing.",
		Args:  cobra.NoArgs,
		RunE: func(_ *cobra.Command, _ []string) error {
		...
		case "experimental":
			if index+1 < len(args) {
				...
				case "graph":
					if index+2 < len(args) {
						switch args[index+2] {
						case "list", "validate", "evaluate", "explain", "test", "schema":
