...
func TestConformanceClaimNamesEverySurfaceReachingTheEvaluator(t *testing.T) {
	...
	expectedSites := map[string]int{
		...
		filepath.Join("..", "mcp", "tools.go"): 1, // experimental_evaluate
	}
	total := 0
	...
	if !strings.Contains(text, "the six surfaces that reach it") {
		t.Fatalf("CONFORMANCE.md no longer states the enumeration's count as %q; update the sentence and this test together", "the six surfaces that reach it")
	}
}
